...
export function DynamicIslandTOC({
  ...
  useEffect(() => {
      ...
      const validHeadings = elements
        ...
        .map((el, index) => {
          ...
          if (idCounts[finalId] !== undefined) {
            idCounts[finalId]++;
            finalId = `${finalId}-${idCounts[finalId]}`;
          } else {
            idCounts[finalId] = 0;
          }
          ...
          return { id: finalId, text, level, element: el };
        });
    ...
    const timer = setTimeout(getHeadings, 1000);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Show TOC only after scrolling near the first tracked heading
      if (headings.length > 0) {
        const firstHeadingTop = headings[0].element.getBoundingClientRect().top;
        if (firstHeadingTop < window.innerHeight) {
          setShowIsland(true);
        } else {
          setShowIsland(false);
          setIsExpanded(false); // collapse if scrolled back up
        }
      } else {
        ...
        {showIsland && (
          ...
          >
            <motion.div
              ...
              className="relative overflow-hidden border border-white/20 bg-white/5 backdrop-blur-3xl text-white shadow-[0_16px_40px_-12px_rgba(0,0,0,0.5),inset_0_1px_1px_rgba(255,255,255,0.1)] ring-1 ring-white/5"
            >
              {/* Liquid Glass Highlight */}
              <div className="absolute inset-x-0 -top-px mx-auto h-[2px] w-3/4 bg-gradient-to-r from-transparent via-white/40 to-transparent opacity-70 pointer-events-none" />
              ...
              >
                <div className="flex shrink-0 items-center justify-between px-6 pb-3 pt-5">
                  <span className="text-[11px] font-semibold tracking-[0.08em] text-white/50">
                    TABLE OF CONTENTS
                  </span>
                  <button
                    onClick={(e) => {
