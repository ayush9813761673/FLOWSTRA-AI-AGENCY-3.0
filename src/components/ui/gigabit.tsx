...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  for (float i = 0.0; i < 80.0; i++)
  {
    ...
    // rotating axis
    v = normalize(cos(t * 0.25 - z * 0.1 + vec3(0.0, 2.0, 4.0)));

    // rotate p
    p = dot(v, p) * v + cross(v, p);

    // pulsing step
    float zPulse = cos(p.z + t * 10.0) * 0.1;
    float xyComp = length(cos(p.xy)) - 0.6;
    d = 0.5 * length(vec2(zPulse, xyComp));

    z += d;

    o += (cos(i * 0.1 + t + vec4(6.0, 1.0, 2.0, 0.0)) + 1.0)
         / max(d, 1e-3) / max(z, 1e-3);
  }
...
export function ShaderCanvas({
  ...
  return (
    <div style={{ position: "absolute", zIndex: 0, inset: 0, pointerEvents: "none" }}>
